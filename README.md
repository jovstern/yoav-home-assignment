...
- **Browse & filter** the resource list - search by name/type/owner plus provider/environment/criticality filters, combinable.
...
- `components/CreateApplicationDialog.test.tsx` - submit is disabled with no name or no selection, and a valid submit calls through to the store with the right payload.
